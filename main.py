...
from PySide6.QtWidgets import QDialog, QGraphicsPixmapItem, QGraphicsScene, QGraphicsSceneMouseEvent, QGraphicsRectItem
...
def cropAndPredict(pixmap: QPixmap, rect: QGraphicsRectItem | None):
    """
    Crop the given pixmap using the provided rectangle and predict the top 5 classes.
    ...
    outputs = model(image)
    predictions = (outputs.argsort(descending=True).ravel().tolist())[:5]
...
class CustomGraphicsScene(QGraphicsScene):
    ...
    def limitPointPos(self):
        ...
        if self.bottomRight.x() < 0.:
            self.bottomRight.setX(0.)
        if self.bottomRight.x() > self.width():
            self.bottomRight.setX(self.width())
        if self.bottomRight.y() < 0.:
            self.bottomRight.setY(0.)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def submitToModel(self):
        if len(self.graphicScene.items()) == 0:
            QMessageBox.critical(self, "No Image Loaded!", "Load an image first, then try again.")
            return
        ...
        pixmap, textPred = cropAndPredict(pixmap.pixmap(), rectangle)
        ...
        resultWindow.label.setText(f"Top 5 predictions: {str(textPred)[1:-1]}")
        resultWindow.show()
        resultWindow.graphicsView.fitInView(resultWindow.graphicsScene.sceneRect(), Qt.AspectRatioMode.KeepAspectRatio)

    def opendialog(self):
        fileSelect = QFileDialog(self)
        fileSelect.setFileMode(QFileDialog.FileMode.ExistingFile)
        # A mouthful...
        fileSelect.setDirectory(QStandardPaths.standardLocations(QStandardPaths.StandardLocation.DesktopLocation)[-1])
        fileSelect.show()
        ...
        if fileSelect.exec():
            ...
            if len(chosenFile) > 1:
                QMessageBox.information(self, "Too many images selected", "Please select one image only.")
...
def extract_pixmap(source_pixmap: QPixmap, rect: QRect):
    ...
    painter.drawPixmap(0, 0, source_pixmap, rect.x(), rect.y(), rect.width(), rect.height())
